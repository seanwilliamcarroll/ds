...
There's a wrinkle, though. Looking at the full range of sizes, IndexArena is actually ~20-30% *slower* than PtrTrie at 512 and 4K, roughly tied at 32K, and only pulls ahead decisively at 64K. The disassembly[^2] reveals the cost: IndexArena pays a `madd` (multiply-add by 216) every iteration to convert an index to an address. Since 216 isn't a power of 2, the compiler can't reduce this to a shift — it's a real multiply. PtrTrie skips this entirely: the pointer *is* the address.
...
But the `cbz` observation raised a question worth testing: what if the null check *could* use `cbz`?
